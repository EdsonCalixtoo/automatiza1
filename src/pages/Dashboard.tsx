...
const Dashboard = () => {
  const [activeTab, setActiveTab] = useState<"overview" | "orders" | "products" | "coupons" | "sellers" | "diagnostico">("overview");
  ...
  const { products, coupons, deleteProduct, deleteCoupon } = useProducts();
...
<body>
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    const colors: { [key: string]: { bg: string; text: string; icon: any } } = {
      pendente: { bg: "bg-yellow-100", text: "text-yellow-800", icon: <Clock className="w-4 h-4" /> },
      confirmado: { bg: "bg-blue-100", text: "text-blue-800", icon: <CheckCircle2 className="w-4 h-4" /> },
      enviado: { bg: "bg-purple-100", text: "text-purple-800", icon: <Package className="w-4 h-4" /> },
      entregue: { bg: "bg-green-100", text: "text-green-800", icon: <CheckCircle2 className="w-4 h-4" /> },
      cancelado: { bg: "bg-red-100", text: "text-red-800", icon: <AlertCircle className="w-4 h-4" /> },
    };
                          ...
                          <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                            ...
                            <div className="space-y-2">
                              ...
                              <div>
                                ...
                                <p className="text-white text-sm font-semibold">{order.paymentMethod === "pix" ? "🔐 PIX" : "💳 Cartão"}</p>
                              </div>
                            </div>
                          </div>
                        </div>
