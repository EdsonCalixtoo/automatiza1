...
export default function Checkout() {
  ...
  const validateForm = (): boolean => {
    ...
    // Validação de pagamento - Simplificada para Redirecionamento
    return true;
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    // A submissão real agora é tratada pelo componente MercadoPagoPayment (Brick)
  };

  const itemsSubtotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const finalTotal = Number((itemsSubtotal + shippingCost - couponDiscount).toFixed(2));
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="space-y-6">
                  ...
                  <div className="group bg-gradient-to-br from-white via-amber-50/20 to-white rounded-3xl p-8 border-2 border-amber-200/50 backdrop-blur-xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:border-amber-300/80"
                    ...
                    <div key={`payment-container-${orderId}`} className="p-4 bg-white rounded-3xl border-2 border-amber-200/50 shadow-inner w-full overflow-hidden">
                      {finalTotal > 0 ? (
                        <MercadoPagoPayment
                          amount={finalTotal}
                          ...
                          onPaymentSuccess={async (data) => {
                            ...
                            try {
                              ...
                              const order = await createOrder({
                                ...
                                total: finalTotal,
                                metodo_pagamento: data.payment_method_id === "pix" ? "pix" : "cartao",
                                cupom: couponCode || undefined,
                              });

                              if (data.payment_method_id === "pix") {
                                // 2. Salvar dados do PIX no pedido
                                ...
                                await updateOrderStatus(order.id, "aguardando_pagamento", {
                                  pix_code: pixCode,
                                ...
                                // 3. Cartão aprovado
                                await updateOrderStatus(order.id, "pago", {
                                  mp_payment_id: String(data.id),
                                });
                        ...
                        <div className="py-12 text-amber-600 font-bold flex flex-col items-center gap-2">
